...
class C {
    public:
        ...
        void func1(A* aa)       { printf("C pointer func1\n"); }
        void func2(A const* aa) { printf("C pointer func2\n"); }
};

struct D : public RefCountedObject
{
    void fun () { printf("fun In ptrtest\n"); }
};

struct E
{
    RefCountedObjectPtr <D> d; // holds a reference to D
};

void ptrtest (RefCountedObjectPtr <D> d)
{
    d->fun ();
}
...
struct F
{
    void funF () { printf("funF In ptrtestF\n"); }
};
...
RefCountedPtr <F> createF ()
{
    return new F;
}

void ptrtestF (RefCountedPtr <F> f)
{
    f->funF ();
}
	...
	getGlobalNamespace (L)
		.beginNamespace ("test")
			...
			.endClass ()
                        .beginClass<C>("C")
                .addConstructor <void (*) (void)> ()
                                .addFunction ("func1", &C::func1)
                                .addFunction ("func2", &C::func2)
                        .endClass ()
                        .beginClass <D> ("D")
                .addConstructor <void (*) (void), RefCountedObjectPtr <D> > ()
                                .addFunction ("fun", &D::fun)
                        .endClass ()
                        .beginClass <E> ("E")
                .addConstructor <void (*) (void), RefCountedPtr <E> > ()
                        .endClass ()
                        .addFunction ("ptrtest", ptrtest)
                        .beginClass<F> ("F")
                .addConstructor <void (*) (void)> ()
                                .addFunction ("funF", &F::funF)
                        .endClass ()
                        .beginClass <G> ("G")
                .addConstructor <void (*) (void), RefCountedPtr <F> > ()
                        .endClass ()
                        .addFunction ("createF", createF)
                        .addFunction ("ptrtestF", ptrtestF)
                        .addFunction ("callfunF", callfunF)
                .endNamespace ();
	//C++ lifetime
	A a;                       
	a.dataMember1 = 42;
