...
public:
    ...
    explicit random_access_skip_list(const Compare & cmp = compare(), const Allocator &alloc = Allocator());

    template <class InputIterator>
    random_access_skip_list(InputIterator first, InputIterator last, const Compare & cmp = compare(), const Allocator &alloc = Allocator());

    random_access_skip_list(const random_access_skip_list &other);
    random_access_skip_list(const random_access_skip_list &other, const Compare & cmp, const Allocator &alloc);
...
template <class T, class C, class A, class LG>
inline
random_access_skip_list<T,C,A,LG>::random_access_skip_list(const compare & cmp, const allocator_type &alloc_)
:   impl(cmp, alloc_)
{
}
...
template <class InputIterator>
inline
random_access_skip_list<T,C,A,LG>::random_access_skip_list(InputIterator first, 
    InputIterator last, 
    const compare &cmp,
...
:   impl(other.impl.less, other.get_allocator())
{    
...
inline
typename random_access_skip_list<T,C,A,LG>::iterator
random_access_skip_list<T,C,A,LG>::lower_bound(const value_type &value)
{
...
inline
typename random_access_skip_list<T,C,A,LG>::const_iterator
random_access_skip_list<T,C,A,LG>::lower_bound(const value_type &value) const
{
...
inline
typename random_access_skip_list<T,C,A,LG>::iterator
random_access_skip_list<T,C,A,LG>::upper_bound(const value_type &value)
{
    node_type *node = impl.find_first(value);
    if (node == impl.one_past_front()) node = node->next[0];
...
inline
typename random_access_skip_list<T,C,A,LG>::const_iterator
random_access_skip_list<T,C,A,LG>::upper_bound(const value_type &value) const
{
    const node_type *node = impl.find_first(value);
    if (node == impl.one_past_front()) node = node->next[0];
    while (impl.is_valid(node) && detail::equivalent(node->value, value, impl.less))
...
std::pair
    <
        ...
        typename random_access_skip_list<T,C,A,LG>::iterator
    >
random_access_skip_list<T,C,A,LG>::equal_range(const value_type &value)
{
...
std::pair
    <
        ...
        typename random_access_skip_list<T,C,A,LG>::const_iterator
    >
random_access_skip_list<T,C,A,LG>::equal_range(const value_type &value) const
{
    return std::make_pair(lower_bound(value), upper_bound(value));
}
...
public:
    ...
    rasl_impl(const Compare &cmp = compare_type(), const Allocator &alloc = Allocator());
    ~rasl_impl();
...
rasl_impl<T,C,A,LG>::find_first(const value_type &value) const
{
...
template <class T, class C, class A, class LG>
inline
rasl_impl<T,C,A,LG>::rasl_impl(const compare_type &cmp, const allocator_type &alloc_)
:   alloc(alloc_),
    less(cmp),
    levels(0),
